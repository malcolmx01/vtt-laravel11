...
use App\Models\Vtt\PrHeader;
use App\Models\Vtt\PrDetail;
...
class PRPrintableExport implements FromView, ShouldAutoSize, WithColumnWidths, WithEvents,  WithStyles, WithDrawings
{
    use Exportable;
...
//                $event->sheet->getDelegate()->getStyle('A10:H11')->getAlignment()->setVertical('center');
    ...
    public function view(): View
    {
        ...
        $PrHeader = PrHeader::with('prdetail', 'office', 'prepared', 'approved')->find($this->q);

        $prDetailItems = collect(PrDetail::with('category' )
            ->with('items' )
            ->where('pr_header_id', $this->q)
            ->select('id', 'pr_header_id', 'pr_no', 'item_id', 'unit_cost as item_unit_cost',  'item_description as item', 'category_id', 'unit_id as item_unit', 'quantity_requested as item_total_qty',
                DB::raw('(quantity_requested * unit_cost) as item_total_cost'))
            ->get()->keyBy('id')->toArray())->groupBy('category.category');

        $this->rowCount = $PrHeader->prdetail()->count() +  ($prDetailItems->count() * 2);
        $this->headerCount = $prDetailItems->count();
        // Count per category items
        $sql = collect(PrDetail::with('category' )
            ->where('pr_header_id', $this->q)
            ->select('id', 'pr_header_id', 'pr_no', 'item_id', 'unit_cost as item_unit_cost',  'item_description as item', 'category_id', 'unit_id as item_unit', 'quantity_requested as item_total_qty',
                DB::raw('(quantity_requested * unit_cost) as item_total_cost'))
            ->get());
        ...
        return view('exports.vtt.pr_printable', [
            'prHeader' => $PrHeader,
            'prDetailItems' => $prDetailItems,
        ]);
