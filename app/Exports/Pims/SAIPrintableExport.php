...
class SAIPrintableExport implements FromView, ShouldAutoSize, WithColumnWidths, WithEvents,  WithStyles, WithDrawings
{
    use Exportable;
...
//                $event->sheet->getDelegate()->getStyle('A10:H11')->getAlignment()->setVertical('center');
    ...
    public function view(): View
    {
        ...
        $SaiHeader = SaiHeader::with('saidetail')->with('office')->find($this->q);

        $saiDetailItems = collect(SaiDetail::with('category' )
            ->with('items' )
            ->where('sai_header_id', $this->q)
            ->select('id', 'sai_header_id', 'sai_no', 'item_id', 'unit_cost as item_unit_cost',  'item_description as item', 'category_id', 'unit_id as item_unit', 'quantity_requested as item_total_qty',
                DB::raw('(quantity_requested * unit_cost) as item_total_cost'))
            ->get()->keyBy('id')->toArray())->groupBy('category.category');

        $this->rowCount = $SaiHeader->saidetail()->count() +  ($saiDetailItems->count() * 2);
        $this->headerCount = $saiDetailItems->count();
        // Count per category items
        $sql = collect(SaiDetail::with('category' )
            ->where('sai_header_id', $this->q)
            ->select('id', 'sai_header_id', 'sai_no', 'item_id', 'unit_cost as item_unit_cost',  'item_description as item', 'category_id', 'unit_id as item_unit', 'quantity_requested as item_total_qty',
                DB::raw('(quantity_requested * unit_cost) as item_total_cost'))
            ->get());

        $categoryItemCount = $sql->groupBy('category.category')->map(function ($row) {
            return $row->count();
        });

        $this->categoryItemCount = $categoryItemCount->toArray();

        return view('exports.vtt.sai_saiintable', [
            'saiHeader' => $SaiHeader,
            'saiDetailItems' => $prDetailItems,
        ]);
...
//            $sheet->getStyle('A'.($totalRow).':F'.($totalRow))->applyFromArray($styleArray);
            $sheet->getStyle('A'.($totalRow).':F'.($totalRow))->getAlignment()->setHorizontal(\PhpOffice\PhpSsaieadsheet\Style\Alignment::HORIZONTAL_RIGHT);
