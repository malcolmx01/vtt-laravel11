<table cellspacing="3" bgcolor="#000000" style="border: 1px solid #000;">
    <thead>
        ...
        <tr>
            <td colspan="19"><h1><strong>PROJECT PROCUREMENT MANAGEMENT PLAN (PPMP) {{$ppmpHeader->app_year}}</strong></h1></td>
        </tr>
        <tr>
            <td colspan="19"></td>
        </tr>
        <tr>
            ...
            <td colspan="18"><strong>{{$ppmpHeader->office->dept_name ?? ''}}</strong> </td>
        </tr>
        <tr>
            <td colspan="1">F/P/P Code:</td>
            <td colspan="18"><strong>{{$ppmpHeader->office_id ?? ''}}</strong> </td>
        </tr>
        <tr>
            <td colspan="1">Control No.:</td>
            <td colspan="18"><strong>{{$ppmpHeader->plan_control_no ?? ''}}</strong> </td>
        </tr>
        <tr>
    ...
    @forelse($PpmpDetailItems as $itemKey => $itemDetails)
    ...
    @endforelse
    @if(!empty($ppmpHeader->planned_amount))
    ...
    <tr>
        ...
        <td colspan="2">{{ \Carbon\Carbon::parse($ppmpHeader->date_submitted)->format('M d Y h:i A') }} </td>
    </tr>
    <tr>
        <td></td>
        <td colspan="5"><strong>{{ strtoupper($ppmpHeader->prepared_name) }}</strong></td>
        <td colspan="4"><strong>{{ strtoupper($ppmpHeader->approved_name) }}</strong></td>
        <td colspan="27"></td>
        <td colspan="2"></td>
        <td colspan="2"></td>
    </tr>
    <tr>
        <td></td>
        <td colspan="5">{{ strtoupper($ppmpHeader->prepared_designation) }}</td>
        <td colspan="4">{{ strtoupper($ppmpHeader->approved_designation) }}</td>
        <td colspan="27"></td>
        <td colspan="2"></td>
        <td colspan="2"></td>
    </tr>
    <tr>
        <td></td>
        <td colspan="5">{{ strtoupper($ppmpHeader->prepared_office) }}</td>
        <td colspan="31">{{ strtoupper($ppmpHeader->approved_office) }}</td>
        <td colspan="2"></td>
        <td colspan="2"></td>
    </tr>
    </tfoot>
