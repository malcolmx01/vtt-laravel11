<div style="position:relative">
    <div class="input-group input-group-solid">
        ...
        <input wire:model.debounce.500ms="search" id="{{ $sfId }}" class="form-control" type="text"  placeholder="{{$placeholder ?? 'Search supplier'}}" aria-label="search" aria-describedby="search"  @if($show) disabled @endif />
        @if(!empty($search))
                    ...
                    @foreach($searchResultItems as $sri)
                        <li class="list-group-item list-group-item-action">
                            <a href="#" class="d-block pe-auto" style="margin: 0 !important;" wire:click="searchSelectItem({{$sri->id}})">
                                <strong>{{ $sri->fuel_depot ?? '' }}</strong><br/>
                                <span>{{ $sri->fuel_depot_location ?? '' }} </span>&nbsp;&nbsp;<br/>
                                <span class="text-muted">{{$sri->fuel_depot_provider_name ?? ''}} </span>
                            </a>
                        </li>
                    @endforeach
                </ul>
