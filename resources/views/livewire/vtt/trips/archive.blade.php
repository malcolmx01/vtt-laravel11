<div class="card card-p-35 card-flush">
    ...
    <div class="card-header align-items-center gap-2 gap-md-5 pt-15 px-10 pt-lg-15 px-lg-15">
        <!--begin::card-title-->
        <h3 class="card-title">Trip Ticket Archive</h3>
        <!--end::card-title-->
    ...
    <div class="card-body pe-15 px-10 pe-lg-15 px-lg-15">
            ...
            <div class="col-md-4">
                <div class="input-group input-group-solid mb-5">
                    ...
                    <select wire:model="orderBy" class="form-select form-select-solid"  id="orderBy">
                        ...
                        <option value="purpose_trip_details">Purpose / Trip Details</option>
                        <option value="person_in_charge_name">Driver</option>
                    </select>
        ...
        <div class="table-responsive">
            <table class="table table-row-dashed table-row-gray-200 align-middle gs-7 gy-4">
                ...
                <tr class="fw-bolder fs-6 text-gray-900">
                    ...
                    <th nowrap>Depature</th>
                    <th nowrap>Origin</th>
                    <th nowrap>Destination</th>
                    <th nowrap>Purpose / Trip Details</th>
                    <th nowrap>Driver</th>
                    <th nowrap>Status</th>
                ...
                @foreach ($trips as $data)
                    <tr wire:key="{{ $loop->index }}">
                        <th>{{ (($trips->currentPage() * $trips->perPage()) - $trips->perPage()) + $loop->iteration }} {{--  $categoryRow->classification()->count() --}}</th>
                         <td nowrap>{{ $data->vehicle_plate_no }}</td>
                        <td nowrap>{{ $data->departure ? \Carbon\Carbon::parse($data->departure)->format('M d, Y - H:i:s') : '' }}</td>
                        <td nowrap>{{ $data->origin }}</td>
                        <td nowrap>{{ $data->destination }}</td>
                        <td nowrap>{{\Illuminate\Support\Str::limit(strip_tags($data->purpose_trip_details) ?? '',60,' ...')}}</td>
        ...
        <div class="row">
            <div class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
                ...
                <div class="dataTables_info" role="status" aria-live="polite">Showing {{$trips->firstItem()}} to {{$trips->lastItem()}} out of {{$trips->total()}} records</div>
            </div>
