<div class="card card-p-35 card-flush">
    ...
    <div class="card-header align-items-center gap-2 gap-md-5 pt-15 px-10 pt-lg-15 px-lg-15">
        <!--begin::card-title-->
        <h3 class="card-title">Vehicle Archive</h3>
        <!--end::card-title-->
    ...
    <div class="card-body pe-15 px-10 pe-lg-15 px-lg-15">
            ...
            <div class="col-md-4">
                <div class="input-group input-group-solid mb-5">
                    ...
                    <select wire:model="orderBy" class="form-select form-select-solid"  id="orderBy">
                        ...
                        <option value="conduction_sticker">Conduction Sticker</option>
                        <option value="make">Make</option>
                        <option value="model">Model</option>
                        <option value="year">Year</option>
                        <option value="color">Color</option>
                        <option value="assigned_office">Assigned Office</option>
                        <option value="CR_no">CR No.</option>
                        <option value="OR_no">OR No/</option>
                        <option value="mode_of_acquisition">Mode of Acquisition</option>
                        <option value="supplier_origin">Supplier / Origin</option>
                    </select>
        ...
        <div class="table-responsive">
            <table class="table table-row-dashed table-row-gray-200 align-middle gs-7 gy-4">
                ...
                <tr class="fw-bolder fs-6 text-gray-900">
                    <th nowrap>#</th>
                    <th nowrap>Plate No</th>
                    <th nowrap>Conduction Sticker</th>
                    <th nowrap>Make</th>
                    <th nowrap>Model</th>
                    <th nowrap>Year Model</th>
                    <th nowrap>Color</th>
                    <th nowrap>Assigned Office</th>
                    <th nowrap>CR No.</th>
                    <th nowrap>OR No.</th>
                    <th nowrap>Mode of Acquisition</th>
                    <th nowrap>Supplier / Origin</th>
                ...
                @foreach ($vehicles as $data)
                    <tr wire:key="{{ $loop->index }}">
                        <th>{{ (($vehicles->currentPage() * $vehicles->perPage()) - $vehicles->perPage()) + $loop->iteration }} {{--  $categoryRow->classification()->count() --}}</th>
                         <td nowrap>{{ $data->vehicle_plate_no }}</td>
                        <td nowrap>{{ $data->conduction_sticker ? $data->conduction_sticker : '' }}</td>
                        <td nowrap>{{ $data->make }}</td>
                        <td nowrap>{{ $data->model }}</td>

                        <td nowrap>{{ $data->year }}</td>
                        <td nowrap>{{ $data->color }}</td>
                        <td nowrap>{{ $data->assigned_office }}</td>

                        <td nowrap>{{ $data->CR_no }}</td>
                        <td nowrap>{{ $data->OR_no }}</td>

                        <td>
                            <span>{{optional($data->ModeOfAcquisition)->mode ?? 'none'}}</span>
                        </td>
                        <td nowrap>{{ $data->supplier_origin }}</td>

                        <td nowrap>{{\Illuminate\Support\Str::limit(strip_tags($data->remarks) ?? '',60,' ...')}}</td>
        ...
        <div class="row">
            <div class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
                ...
                <div class="dataTables_info" role="status" aria-live="polite">Showing {{$vehicles->firstItem()}} to {{$vehicles->lastItem()}} out of {{$vehicles->total()}} records</div>
            </div>
