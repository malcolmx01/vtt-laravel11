    ...
    <div class="modal-dialog modal-dialog-scrollable modal-primary modal-xl  {{$maximize ?? ''}}" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="formModalLabel">{{$crud ? ucfirst($crud) : 'Create' }} | <span class="text-muted fw-normal">Vehicle</span></h5>
                <div>
                    @if($crud == 'edit' || $crud == 'show')
                        @role('admin|divisionhead|departmenthead|approver')
                        <button type="button" class="btn btn-sm {{!empty($vehicle->status_date) ? 'btn-light-success ' : 'btn-light-primary ' }} x-button py-1 px-4" wire:click="setstatus({{$vehicle->id}})">
                            ...
                            <!--end::Svg Icon-->
                            {{!empty($vehicle->status_date) ? 'Edit ' : 'Set ' }}
                            Status
                        </button>
                        @endrole
                    @endif
                                ...
                                <div class="card-body pt-16">
                                    <div class="row mb-5">
                                        <div class="col-md-2 mb-3">
                                            ...
                                            <input wire:model="vehicle.vehicle_plate_no" type="text" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.vehicle_plate_no')) is-invalid @endif" placeholder="Vehicle Plate No." required @if($disable) disabled @endif>
                                            @error('vehicle.vehicle_plate_no') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-md-2 mb-3">
                                            <label class="form-label">Conduction Sticker</label>
                                            <input wire:model="vehicle.conduction_sticker" type="text" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.conduction_sticker')) is-invalid @endif" placeholder="Conduction sticker" required @if($disable) disabled @endif>
                                            @error('vehicle.conduction_sticker') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-md-2 mb-3">
                                            <label class="form-label required">Make</label>
                                            <input wire:model="vehicle.make" type="text" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.make')) is-invalid @endif" placeholder="Honda, Toyota, BMW..." required @if($disable) disabled @endif>
                                            @error('vehicle.make') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-md-2 mb-3">
                                            <label class="form-label">Model</label>
                                            <input wire:model="vehicle.model" type="text" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.model')) is-invalid @endif" placeholder="Fortuner, Alphard, Land Cruiser..." required @if($disable) disabled @endif>
                                            @error('vehicle.model') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-md-2 mb-3">
                                            <label class="form-label required">Year Model</label>
                                            <input wire:model="vehicle.year" type="number" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.year')) is-invalid @endif" placeholder="Year Model" required @if($disable) disabled @endif>
                                            @error('vehicle.year') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-md-2 mb-3">
                                            <label class="form-label required">Color</label>
                                            <input wire:model="vehicle.color" type="text" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.color')) is-invalid @endif" placeholder="Color" required @if($disable) disabled @endif>
                                            @error('vehicle.color') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
                                    <div class="row mb-5">
                                        <div class="col-md-4 mb-3">
                                            @if(!empty($ModeAcquisition))
                                                <label class="form-label">Mode of Acquisition</label>
                                                <select wire:model="vehicle.status" class="form-select"  @if($disable) disabled @endif>
                                                    <option> Mode of Acquisition </option>
                                                    @foreach($ModeAcquisition as $mode)
                                        ...
                                        <div class="col-md-4 mb-3">
                                            <label class="form-label required">Supplier/Origin</label>
                                            <input wire:model="vehicle.supplier_origin" type="text" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.supplier_origin')) is-invalid @endif" placeholder="Supplier / Origin" required @if($disable) disabled @endif>
                                            @error('vehicle.supplier_origin') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-md-4 mb-3">
                                            <label class="form-label required">Certification of Registration (CR) No.</label>
                                            <input wire:model="vehicle.CR_no" type="text" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.CR_no')) is-invalid @endif" placeholder="LTO Certification of Registration No." required @if($disable) disabled @endif>
                                            @error('vehicle.CR_no') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                    ...
                                    <div class="row mb-5">
                                        <div class="col-md-4 mb-3">
                                            ...
                                            <input wire:model="vehicle.chassis_no" type="text" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.chassis_no')) is-invalid @endif" placeholder="Chassis No." required @if($disable) disabled @endif>
                                            @error('vehicle.chassis_no') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-md-4 mb-3">
                                            <label class="form-label">Engine No.</label>
                                            <input wire:model="vehicle.engine_no" type="text" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.engine_no')) is-invalid @endif" placeholder="Engine No." required @if($disable) disabled @endif>
                                            @error('vehicle.engine_no') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>

                                        <div class="col-md-4 mb-3">
                                            <label class="form-label required">Official Receipt (OR) No.</label>
                                            <input wire:model="vehicle.OR_no" type="text" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.OR_no')) is-invalid @endif" placeholder="LTO Official Receipt No." required @if($disable) disabled @endif>
                                            @error('vehicle.OR_no') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
                                    <div class="row mb-5">
                                        <div class="col-md-5 mb-3">
                                            <label class="form-label required">Assigned Office/s</label>
                                            <input wire:model="vehicle.assigned_office" type="text" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.assigned_office')) is-invalid @endif" placeholder="Assigned Office/s authorized to use the Vehicle" required @if($disable) disabled @endif>
                                            @error('vehicle.assigned_office') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-md-5 mb-3">
                                            <label class="form-label required">Person in Charge</label>
                                            @if(!$disable)
                                            ...
                                            @endif
                                            @if(!empty($vehicle->person_in_charge_id))
                                                <div class="position-relative border p-3 mt-5">
                                                    ...
                                                    <div class="text-center pt-3">
                                                        <p class="border-bottom text-uppercase">
                                                            <strong>{{$vehicle->person_in_charge_name ?? ''}}</strong><br/>
                                                            {{$vehicle->person_in_charge_designation ?? ''}}
                                                        </p>
                                                        <p>
                                                            {{$vehicle->person_in_charge_office ?? ''}}
                                                        </p>
                                                    </div>
                                                </div>
                                            @endif
                                            @error('vehicle.person_in_charge_id') <span class="invalid-feedback d-block">{{ $message }}</span>@enderror
                                        </div>
                                        <div class="col-md-2 mb-3">
                                            @if(!empty($DocStatus))
                                                <label class="form-label">Status</label>
                                                <select wire:model="vehicle.status" class="form-select" disabled>
                                    ...
                                    <div class="row mb-5">
                                        <div class="col-md-12 mb-3">
                                            <div class="vehicle_remarks" wire:ignore>
                                                <label class="form-label">Remarks</label>
                                                <textarea wire:model="vehicle.remarks"
                                                          class="min-h-fit h-40 "
                                                          name="remarks"
                                                          id="vehicle_remarks">
                                                </textarea>
                                            </div>
                                            @error('vehicle.remarks') <span class="invalid-feedback d-block">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
            ...
            <div class="modal-footer">
                ...
                @elseif($crud == 'show')
                    <button class="btn btn-primary close-modal px-5" wire:click.prevent="edit({{$vehicle->id}})" wire:loading.attr="disabled" @if($disableeditdetails) disabled @endif><div wire:loading wire:target="edit"><span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span></div> Edit</button>
                @elseif($crud == 'edit')
    ...
    <script>
        $(document).ready(function () {
            ...
            ClassicEditor
                ...
                .create(document.querySelector('#vehicle_remarks'),{
                    toolbar: {
                        items: [
                            'heading', '|',
                            'fontfamily', 'fontsize', '|',
                ...
                .then(editor => {
                    editor.model.document.on('change:data', () => {
                        @this.set('vehicle.remarks', editor.getData());
                    });
