...
<div>
    ...
    @include('livewire.vtt.vehicles.archive')
    @include('livewire.vtt.vehicles.form')

    <div>
        <livewire:vtt.vehicles-set-status :wire:key="'setstat-'.$setstatusid"/>
    </div>
    {{--<div>--}}
        {{--<livewire:vtt.ris-attach-item :wire:key="$addItemUniqid"/>--}}
    {{--</div>--}}
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        window.livewire.on('showDeleteConfirmation', data => {
            swal.fire({
            html: "Are you sure to delete Vehicle with Plate No." + data.detail.vehicle_plate_no + " ?",
            icon: 'warning',
        ...
        window.livewire.on('showDeleteDetailsConfirmation', data => {
            swal.fire({
            html: "Are you sure to delete (" + data.detail.vehicle_plate_no + ") with id no.: "+ data.detail.id +" ?",
            icon: 'warning',
            buttonsStyling: false,
            confirmButtonText: "Yes, Delete!",
            showCancelButton: true,
