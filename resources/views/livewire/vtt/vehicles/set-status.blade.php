        ...
        <div class="modal-dialog modal-primary">
            <div class="modal-content">
                    ...
                    <form>
                        <div class="row mb-5">
                            <div class="col-md-12 mb-3">
                                ...
                                <div wire:ignore >
                                    <select wire:model="vehicle.status" data-dropdown-parent="#kt_modal_setstatus" tabindex="-1" data-control="select2" data-placeholder="Select Status" class="form-select form-select-solid @if($errors->has('vehicle.status')) is-invalid @endif" id="vehicle_status" >
                                        <option value="0"> Select Status </option>
                                        @foreach($DocStatus as $status)
                                            <option  value="{{$status->id}}">{{$status->status}}</option>
                                        @endforeach
                                    </select>
                                    @error('vehicle.status') <span class="invalid-feedback">{{ $message }}</span>@enderror
                                </div>
                            </div>
                            <div class="col-md-12 mb-3">
                                <label class="form-label">Date and Time:</label>
                                <input wire:model="vehicle.status_date" type="datetime-local" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.status_date')) is-invalid @endif" id="vehicle.status_date" required @if($disable) disabled @endif>
                                @error('vehicle.status_date') <span class="invalid-feedback">{{ $message }}</span>@enderror
                            </div>
                            <div class="col-md-12 mb-3">
                                    ...
                                    <div class="position-relative border p-3 mt-5">
                                        ...
                                        <div class="text-center pt-3">
                                            <p class="border-bottom text-uppercase">
                                                <strong>{{$vehicle->setby_name ?? ''}}</strong><br/>
                                                {{$vehicle->setby_designation ?? ''}}
                                            </p>
                                            <p>
                                                {{$vehicle->setby_office ?? ''}}
                                            </p>
                                        </div>
                                    </div>
                                @endif
                        ...
                        <div class="row mb-5">
                            <div class="col-md-12 mb-3">
                                ...
                                <textarea wire:model="vehicle.setby_remarks" rows="2" class="form-control @if(!$disable) form-control-solid @endif @if($errors->has('vehicle.remarks')) is-invalid @endif" @if($disable) disabled @endif></textarea>
                                @error('vehicle.remarks') <span class="invalid-feedback">{{ $message }}</span>@enderror
                            </div>
                        </div>
                    </form>
                    <!-- ********************************************************************************** -->
                ...
                <div class="modal-footer">
                    ...
                    @elseif($crud == 'edit')
                        <button class="btn btn-primary px-5" wire:click.prevent="update({{$vehicle->id}})" wire:loading.attr="disabled"><div wire:loading wire:target="update"><span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span></div> Update </button>
                    @endif
    ...
    <script>
        $(document).ready(function () {
            window.livewire.on('setby_resetItem', event => {
                $('#vehicle_status').select2("enable");
                $('#vehicle_status').select2({minimumResultsForSearch: 10 }).val(null).trigger('change');
                $('#vehicle_status').on('change', function (e) {
                    @this.set('vehicle.status', e.target.value, true);
                });
            });

            window.livewire.on('setby_loadDefaultValue', data => {
                $('#vehicle_status').select2("enable");
                $('#vehicle_status').select2().val(data.status).trigger('change');
                $('#vehicle_status').on('change', function (e) {
                @this.set('vehicle.status', e.target.value, true);
                });
                ...
                @this.set('vehicle.setby', e.target.value);
                });
            });

            window.livewire.on('setby_loadValueAndDisable', data => {
                $('#vehicle_status').select2("enable",false);
                $('#vehicle_status').select2().val(data.status).trigger('change');
            });
